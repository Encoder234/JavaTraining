...
public class ATMTransaction {
	...
	public static void checkBalance(String accountNumber, double balance) { 
		...
		try {
			...
			int acctNum = Integer.parseInt(accountNumber);
			char accountType = accountNumber.charAt(0);
			
			if (accountType == '1') System.out.println("Account Type: Savings");
			else if (accountType == '2') System.out.println("Account Type: Checking");
			else System.out.println("Account Type: Unknown");
		...
		} catch (NumberFormatException e) { System.out.println("Error: Invalid account number format! Account numbers must be numeric.");
		} catch (StringIndexOutOfBoundsException e) {System.out.println("Error: Account number is empty or invalid!");
		} finally {
			System.out.println("\n========== RECEIPT ==========");
